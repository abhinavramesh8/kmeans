...
__device__ float distance2(float *x, float *y, int dims) {
    float sq_dist = 0.0f;
...
__device__ void add_vector2(float *to_vect, float *from_vect, int dims) {
    for (int i = 0; i < dims; i++) {
        atomicAdd(to_vect + i, from_vect[i]);
    }
}

__global__ void assign_centroid2(float *pt_eles, float *cent_eles, int *labels, 
								 int n_points, int n_clusters, int dims) {
    extern __shared__ float temp_cent_eles[];
    if (threadIdx.x < n_clusters) {
        int i = threadIdx.x * dims, iters = 0;
        while (iters < dims) {
            temp_cent_eles[i] = cent_eles[i];
            i++;
            iters++;
        }
    }
    ...
    if (pt_idx < n_points) {
        ...
        for (int i = 0; i < n_clusters; i++) {
            float dist = distance2(point, temp_cent_eles + dims * i, dims);
            if (dist < min_dist) {
                min_dist = dist;
                min_idx = i;
            }
...
__global__ void zero_centroid_element2(float *cent_eles, int n_cent_eles) {
...
__global__ void add_point_to_centroid2(float *cent_eles, float *pt_eles, int *labels, 
									   int *counts, int n_clusters, int n_points, 
									   int dims) {
    extern __shared__ float temp_cent_eles[];  
    if (threadIdx.x < n_clusters) {
        int i = threadIdx.x * dims, iters = 0;
        while (iters < dims) {
            temp_cent_eles[i] = 0.0;
            i++;
            iters++;
        }
    }
    ...
    int pt_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (pt_idx < n_points) {
        add_vector2(temp_cent_eles + dims * labels[pt_idx], 
                   pt_eles + pt_idx * dims, dims);
    }
    __syncthreads();
    if (threadIdx.x < n_clusters) {
        int offset = threadIdx.x * dims; 
        add_vector2(cent_eles + offset, temp_cent_eles + offset, dims);
    }
}

__global__ void update_count2(int *counts, int *labels, int n_clusters, int n_points) {
    extern __shared__ int temp_counts[];
    if (threadIdx.x < n_clusters) {
    ...
    int pt_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (pt_idx < n_points) {
        atomicAdd(temp_counts + labels[pt_idx], 1);
    }
    __syncthreads();
    if (threadIdx.x < n_clusters) {
...
__global__ void average_centroid_element2(float *cent_eles, int *counts, 
										  int n_cent_eles, int n_clusters, 
										  int dims) {
    extern __shared__ int temp_counts[];
    if (threadIdx.x < n_clusters) {
        temp_counts[threadIdx.x] = counts[threadIdx.x];
    }
    __syncthreads();
    int cent_ele_idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (cent_ele_idx < n_cent_eles) {
        cent_eles[cent_ele_idx] /= temp_counts[cent_ele_idx / dims];
    }
}

__global__ void centroid_converged2(float *cent_eles, float *old_cent_eles, 
    ...
    if (cent_idx < n_clusters) {
        ...
        float dist = distance2(cent_eles + offset, old_cent_eles + offset, dims);
        if (dist > threshold) {
...
void kmeans_cuda_shmem(kmeans_args_t& args) {
...
//         tran1_diff += diff;
        ...
        assign_centroid2<<<n_blocks_pts, n_threads_per_block, cent_eles_sz>>>(
            dev_point_elements, dev_centroid_elements, dev_labels, args.n_points, 
            args.n_clusters, args.dims);
        
        zero_centroid_element2<<<n_blocks_cent_eles, n_threads_per_block>>>(
            dev_centroid_elements, n_cent_eles);
        
        add_point_to_centroid2<<<n_blocks_pts, n_threads_per_block, cent_eles_sz>>>(
            dev_centroid_elements, dev_point_elements, dev_labels, dev_counts, 
            args.n_clusters, args.n_points, args.dims);
		
		update_count2<<<n_blocks_pts, n_threads_per_block, counts_sz>>>(
			dev_counts, dev_labels, args.n_clusters, args.n_points);
        
        average_centroid_element2<<<n_blocks_cent_eles, n_threads_per_block, counts_sz>>>(
            dev_centroid_elements, dev_counts, n_cent_eles, args.n_clusters, args.dims);
        
        centroid_converged2<<<n_blocks_cents, n_threads_per_block>>>(
